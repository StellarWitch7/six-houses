...
class WitchComponent(private val player: PlayerEntity) : ServerTickingComponent {
    ...
    override fun readFromNbt(tag: NbtCompound, registryLookup: RegistryWrapper.WrapperLookup) {
        val result: DataResult<House> = House.CODEC.get().codec().parse(NbtOps.INSTANCE, tag.get("house"))
    ...
    override fun writeToNbt(tag: NbtCompound, registryLookup: RegistryWrapper.WrapperLookup) {
        house?.let {
            tag.put("house", House.CODEC.get().codec().encodeStart(NbtOps.INSTANCE, it).result().orElseThrow())
        }
    }
